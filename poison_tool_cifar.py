...
def get_train_loader(args):
    ...
    train_loader = DataLoader(train_data, batch_size=args.batch_size*2, shuffle=True)
...
def get_test_loader(args):
    ...
    if (args.dataset == 'CIFAR10'):
        testset = datasets.CIFAR10(root='./CIFAR10', train=False, download=True)
    else:
        raise Exception('Invalid dataset')
...
def get_sorted_backdoor_loader(args, train_data_bad, indices):
    print('==> Preparing train data..')
    tf_train = transforms.Compose([
        transforms.ToPILImage(),
        transforms.RandomCrop(32, padding=4),
...
class DatasetBD(Dataset):
    ...
    def addTrigger(self, dataset, target_label, inject_portion, mode, distance, trig_w, trig_h, trigger_type, target_type):
        ...
        # if mode == 'train' and os.path.exists(f'{self.args.resume_path}/perm_dataset={self.args.dataset}_id={self.args.id}_trig_type={self.args.trigger_type}_target_label={self.args.target_label}_target_type={self.args.target_type}_trig_w={self.args.trig_w}_trig_h={self.args.trig_h}_pr={inject_portion}.npy'):
        #     perm = np.load(f'{self.args.resume_path}/perm_dataset={self.args.dataset}_id={self.args.id}_trig_type={self.args.trigger_type}_target_label={self.args.target_label}_target_type={self.args.target_type}_trig_w={self.args.trig_w}_trig_h={self.args.trig_h}_pr={inject_portion}.npy')
        # else:
        perm = np.random.permutation(len(dataset))[0: int(len(dataset) * inject_portion)]
        # if mode == 'train':
        #     np.save(f'{self.args.resume_path}/perm_dataset={self.args.dataset}_id={self.args.id}_trig_type={self.args.trigger_type}_target_label={self.args.target_label}_target_type={self.args.target_type}_trig_w={self.args.trig_w}_trig_h={self.args.trig_h}_pr={inject_portion}.npy',perm)
    ...
    def selectTrigger(self, img, width, height, distance, trig_w, trig_h, mode, triggerType):
        ...
        # elif triggerType == 'badnet_temp':
        #     img = self._generate_badnet_temp(img)
            
        # elif triggerType == 'blend_temp':
        #     img = self._generate_blend_temp(img)
            
        # elif triggerType == 'refool_temp':
        #     img = self._generate_refool_temp(img)
            
        # elif triggerType == 'TaCT_temp':
        #     img = self._generate_TaCT_temp(img)
            
        # elif triggerType == 'clean_label_temp':
        #     img = self._generate_clean_label_temp(img)
